...
for(i in links[1:4]) {
  la_nacion <- rbind(la_nacion, data.frame(fecha=read_html(i) |> html_elements('.com-date.--twoxs') |> html_text2(), 
                                           titular=read_html(i) |> html_elements('.com-title.--threexl') |> html_text2()))
}
